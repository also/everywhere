...
export default function() {
  ...
  const waysGeojson = JSON.parse(fs.readFileSync(path.join(__dirname, '../app-data/highways-clipped-topo.geojson')));
  ...
  const trips = fs.readdirSync(tripDir).map(tripFilename => {
    ...
    const {features: [{geometry}]} = trip;
    (geometry.type === 'LineString' ? [geometry.coordinates] : geometry.coordinates).forEach(coords => coordCount += coords.length);
  ...
  let n = 0;
  for (const {features: [{geometry}]} of trips) {
    (geometry.type === 'LineString' ? [geometry.coordinates] : geometry.coordinates).forEach(coords => coords.forEach(coord => {
      const nearestWay = waysTree.nearest(coord).data.feature.properties;
      coord[coord.length - 1].push(nearestWay.id);
      if (n++ % 1 === 0) {
        process.stdout.write(`\r\x1b[K${Math.round(n / coordCount * 100)}%\t${nearestWay.name}`);
      }
    }));
  }

  for (const {filename, topo} of trips) {
    fs.writeFileSync(path.join(path.dirname(filename), `${path.basename(filename, '.geojson')}-ways.geojson`), JSON.stringify(topo, null, 2));
  }
}
