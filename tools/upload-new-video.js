...
export default function({_: [dir='video']}) {
  ...
  .then(raw => {
    ...
    if (first) {
      ...
      const upload = client.uploadFile({localFile: path.join(dir, basename), s3Params: {Bucket, Key}});

      console.log(Key);

      return new Promise((resolve, reject) => {
         upload.on('progress', () => process.stdout.write(`${(upload.progressAmount / upload.progressTotal * 100).toFixed(2)}\r`));
