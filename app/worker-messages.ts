...
export const toolFiles = key<
  ...
  void
>('toolFiles');
export const featureSummary = key<undefined, LimitedFeature[]>(
  'featureSummary'
);
export const features = key<undefined, Feature[]>('features');
export const getFeature = key<{ index: number }, Feature>('getFeature');
export const toolFileStatus = key<{ index: number; status: string }, void>(
  'toolFileStatus'
);
export const toolReady = key<{ resultType: 'tiles' }, void>('toolReady');
export const getTile = key<{ x: number; y: number; z: number }, Tile>(
  'getTile'
);
export const renderTileInWorker = key<
  {
